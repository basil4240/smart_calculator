...
/// A widget that represents the keypad for the scientific calculator.
///
/// This widget displays a grid of buttons for numerical input, basic arithmetic
/// operations, and scientific functions. It interacts with the [CalculatorProvider]
/// to process button presses.
class ScientificCalculatorKeyPad extends StatelessWidget {
  /// Creates a [ScientificCalculatorKeyPad] widget.
  const ScientificCalculatorKeyPad({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final double childAspectRatio = (width / 5) / (height / 5);

        return GridView.count(
          crossAxisCount: 5,
          childAspectRatio: childAspectRatio,
          children: <Widget>[
            _buildButton('sin', calculatorProvider, height, context),
            _buildButton('cos', calculatorProvider, height, context),
            _buildButton('tan', calculatorProvider, height, context),
            _buildButton('^', calculatorProvider, height, context),
            _buildButton('sqrt', calculatorProvider, height, context),
            _buildButton('(', calculatorProvider, height, context),
            _buildButton(')', calculatorProvider, height, context),
  ...
  /// Helper method to build a single scientific calculator button.
  ///
